...
fn test_batch_and_streaming_together() {
    ...
    let offset = 1000;
    let stream_result1 = backend.extract_entities_streaming(texts[0], offset).unwrap();
    let stream_result2 = backend.extract_entities_streaming(texts[1], offset + texts[0].len() + 1).unwrap();

...
echo ""
VARIANCE_PATTERNS=$(rg -c "variance.*sum.*/.*len\(\)" --type rust src/eval/task_evaluator.rs 2>/dev/null || echo "0")
BESSEL_PATTERNS=$(rg -c "variance.*sum.*/.*\(.*len\(\)\s*-\s*1" --type rust src/eval/task_evaluator.rs 2>/dev/null || echo "0")
...
echo ""
CI_FUNCTIONS=$(rg -c "compute_confidence|confidence.*interval" --type rust src/eval/task_evaluator.rs 2>/dev/null || echo "0")
EDGE_CHECKS=$(rg -c "n\s*==\s*0|n\s*==\s*1|len\(\)\s*<=\s*1" --type rust src/eval/task_evaluator.rs 2>/dev/null || echo "0")
...
ZERO_CHECKS=$(rg -c "if.*precision.*\+.*recall.*==.*0|if.*tp.*\+.*fp.*==.*0" --type rust src/eval/metrics.rs 2>/dev/null || echo "0")
...
echo ""
CACHE_ACCESS=$(rg -c "per_example_scores_cache" --type rust src/eval/task_evaluator.rs 2>/dev/null || echo "0")
RECOMPUTE_PATTERNS=$(rg -c "recreate.*backend|BackendFactory::create.*confidence" --type rust src/eval/task_evaluator.rs 2>/dev/null || echo "0")
...
echo ""
STRATIFIED=$(rg -c "StratifiedMetrics|by_entity_type" --type rust src/eval/task_evaluator.rs 2>/dev/null || echo "0")
PER_TYPE=$(rg -c "per.*type.*score|by_type.*compute" --type rust src/eval/task_evaluator.rs 2>/dev/null || echo "0")

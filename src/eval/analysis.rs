...
impl ConfusionMatrix {
    ...
    pub fn add(&mut self, predicted: &str, actual: &str) {
        *self.matrix
            ...
            .or_insert(0) += 1;
        *self.predicted_totals.entry(predicted.to_string()).or_insert(0) += 1;
    ...
    pub fn types(&self) -> Vec<String> {
        let mut types: Vec<String> = self.predicted_totals.keys()
    ...
    pub fn precision(&self, entity_type: &str) -> f64 {
        ...
        if predicted == 0 { 0.0 } else { correct as f64 / predicted as f64 }
    }
    ...
    pub fn recall(&self, entity_type: &str) -> f64 {
        ...
        if actual == 0 { 0.0 } else { correct as f64 / actual as f64 }
    }
...
impl ErrorAnalysis {
    ...
    pub fn analyze(
        ...
        for pred in predicted {
            ...
            if let Some((gold_idx, error_type)) = best_match {
                ...
                let context_end = (pred.end + 20).min(text.len());
                ...
                *analysis.counts.entry(error_type).or_insert(0) += 1;
                ...
                // Check if this prediction matched anything correctly
                let matched_correctly = gold.iter().enumerate().any(|(i, g)| {
                    gold_matched[i] && 
                    pred.start == g.start && 
                    pred.end == g.end && 
                ...
                if !matched_correctly {
                    // Spurious prediction (no match found)
                    let context_start = pred.start.saturating_sub(20);
                    let context_end = (pred.end + 20).min(text.len());
        ...
        for (i, g) in gold.iter().enumerate() {
            if !gold_matched[i] {
                ...
                let context_end = (g.end + 20).min(text.len());
    ...
    pub fn summary(&self) -> String {
        ...
        s.push_str(&format!("Error Analysis ({} predictions, {} gold):\n", 
        ...
        for error_type in [
            ...
            s.push_str(&format!("  {:15} {:4} ({:.1}%)\n", 
                format!("{:?}", error_type), count, rate));
        }
...
pub fn build_confusion_matrix(
    predictions: &[(Vec<Entity>, Vec<GoldEntity>)],
) -> ConfusionMatrix {
...
mod tests {
    ...
    fn test_error_analysis() {
        ...
        assert_eq!(
            analysis.counts.get(&ErrorType::BoundaryError).copied().unwrap_or(0),
            1,
            "Expected boundary error for Microsoft/Google overlap"
        );
        // John Smith matches exactly, so no errors from that
        assert_eq!(
            analysis.counts.get(&ErrorType::TypeMismatch).copied().unwrap_or(0),
            0,
    ...
    fn test_error_type_mismatch() {
        ...
        let gold = vec![
            GoldEntity::new("Person", EntityType::Person, 5),
        ];
        
        let analysis = ErrorAnalysis::analyze(text, &predicted, &gold);
        assert_eq!(analysis.counts.get(&ErrorType::TypeMismatch).copied().unwrap_or(0), 1);
    }
    ...
    fn test_error_boundary() {
        ...
        let gold = vec![
            GoldEntity::new("Dr. John Smith", EntityType::Person, 0),
        ];
        
        let analysis = ErrorAnalysis::analyze(text, &predicted, &gold);
        assert_eq!(analysis.counts.get(&ErrorType::BoundaryError).copied().unwrap_or(0), 1);
    }
    ...
    fn test_perfect_match_no_errors() {
        ...
        assert_eq!(analysis.counts.get(&ErrorType::TypeMismatch).copied().unwrap_or(0), 0);
        assert_eq!(analysis.counts.get(&ErrorType::BoundaryError).copied().unwrap_or(0), 0);
        assert_eq!(analysis.counts.get(&ErrorType::Spurious).copied().unwrap_or(0), 0);
        assert_eq!(analysis.counts.get(&ErrorType::Missed).copied().unwrap_or(0), 0);
    }

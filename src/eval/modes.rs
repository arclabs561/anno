...
fn has_sufficient_overlap(start1: usize, end1: usize, start2: usize, end2: usize, min_threshold: f64) -> bool {
...
fn entities_match_with_config(pred: &Entity, gold: &GoldEntity, mode: EvalMode, config: &EvalConfig) -> bool {
    match mode {
        EvalMode::Strict => {
            pred.start == gold.start && pred.end == gold.end && types_match(&pred.entity_type, &gold.entity_type)
        }
        ...
        EvalMode::Partial | EvalMode::Type => {
            has_sufficient_overlap(pred.start, pred.end, gold.start, gold.end, config.min_overlap)
...
mod tests {
    ...
    fn test_partial_with_high_threshold() {
        ...
        let config = EvalConfig::new().with_min_overlap(0.5);
        let results = evaluate_with_config(&predicted, &gold_entities, EvalMode::Partial, &config);
        assert!((results.f1 - 1.0).abs() < 0.001, "0.5 threshold should pass");
